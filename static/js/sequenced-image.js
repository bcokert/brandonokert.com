var sequencedImages = document.querySelectorAll('.sequenced-image');

var icons = {
  next: 'data:image/png;base64,',
  prev: 'data:image/png;base64,',
  toStart: 'data:image/png;base64,',
  toEnd: 'data:image/png;base64,'
};

function createUrlFromBase(urlBase, urlNum) {
  return urlBase + urlNum + '.png';
}

Array.prototype.map.call(sequencedImages, function (root) {
  var urlBase = root.attributes['data-base'].value;
  var infoStrings = Array.prototype.map.call(root.querySelectorAll('p'), function (p) {
    return p.innerHTML;
  });
  var urlMaxNum = infoStrings.length;
  var urlNum = 1;

  var imageArea = document.createElement('img');
  imageArea.className = '-sequenced-image-fragment';
  imageArea.src = createUrlFromBase(urlBase, urlNum);

  var divider = document.createElement('div');
  divider.className = '-divider';

  var textArea = document.createElement('div');
  textArea.innerHTML = infoStrings[urlNum - 1];
  textArea.className = '-textArea';

  var iconSize = "fa-lg";
  if (window.outerWidth > 800) {
    iconSize = "fa-3x";
  } else if (window.outerWidth > 500) {
    iconSize = "fa-2x";
  }

  var leftButton = document.createElement('i');
  leftButton.className = '-control -left -disabled fa fa-angle-left ' + iconSize;
  var onLeft = function() {
    rightButton.className = '-control -right fa fa-angle-right ' + iconSize;
    if (urlNum > 1) {
      urlNum--;
      imageArea.src = createUrlFromBase(urlBase, urlNum);
      if (urlNum === 1) {
        leftButton.className = '-control -left -disabled fa fa-angle-left ' + iconSize;
      }
      textArea.innerHTML = infoStrings[urlNum - 1];
    }
  };
  leftButton.addEventListener('click', onLeft);

  var rightButton = document.createElement('i');
  rightButton.className = '-control -right fa fa-angle-right ' + iconSize;
  var onRight = function() {
    leftButton.className = '-control -left fa fa-angle-left ' + iconSize;
    if (urlNum < urlMaxNum) {
      urlNum++;
      imageArea.src = createUrlFromBase(urlBase, urlNum);
      if (urlNum === urlMaxNum) {
        rightButton.className = '-control -right -disabled fa fa-angle-right ' + iconSize;
      }
      textArea.innerHTML = infoStrings[urlNum - 1];
    }
  };
  rightButton.addEventListener('click', onRight);

  root.appendChild(imageArea);
  root.appendChild(divider);
  root.appendChild(leftButton);
  root.appendChild(rightButton);
  root.appendChild(textArea);
});

document.addEventListener("resize", function () {
  if (window.outerWidth > 800) {
    var controls = document.querySelectorAll('.-control');
    Array.prototype.map.call(controls, function (control) {
      control.className = control.className.split()
    });
  }
});
